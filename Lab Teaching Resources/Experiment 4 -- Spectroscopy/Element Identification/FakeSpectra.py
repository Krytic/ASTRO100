...
for file in os.listdir('elements'):
    if file not in ['..', '.']:
        ...
        print(name)
        if name.lower() not in ['carbon', 'nitrogen', 'oxygen']:
            continue
...
fig, axes = plt.subplots(1, 1, sharex=True, figsize=A4)
...
for species, wavelengths in spectra.items():
    for wl in wavelengths:
        if wl < 380 or wl > 780:
            continue
        rgb = nmToRGB(wl)
        print(alpha[species.lower()])
        axes.axvline(wl, c=rgb, lw=2, alpha=alpha[species.lower()])
    # axes.set_ylabel(species)
    axes.set_yticks([])

    i += 1

axes.set_xlabel("Wavelength in nanometers")

size = axes.yaxis.label.get_fontsize() * 2
...
fig.suptitle("The Emission Spectra of an unknown star", y=0.94, size=size)

plt.savefig('sample_question.pdf')